...
export class Entity {
  ...
  public AssignSlot(slot: number) {
    this.__SLOT_NUM = slot;
  } // Do not call unless you know what you are doing.
  public GetProcessSlot(): number {
    return this.__SLOT_NUM;
  }
  ...
  __INTERNAL_INIT(new_slot: number): void {
    ...
    this.__identifier = btoa(
      TL.Rand(1, 999999999).toString() +
        Game.active_game.__PREVIOUS_UPDATE_MS.toString() +
        Game.entities_created.toString() +
        Game.entities_destroyed.toString(),
    );
    this.OnInit();
  }
  ...
  public __INTERNAL_UPDATE() {
    ...
    if (
      ...
      len > 0 &&
      Math.abs(this.frame) % len <= Math.abs(frame_before) % len
    )
...
export class GameObj extends Entity {
  ...
  override __INTERNAL_UPDATE() {
    ...
    if (
      this.VIEW_EDGE_LIMIT >= 0 &&
      PointOutsideView(this.position.x, this.position.y, this.VIEW_EDGE_LIMIT)
    )
      this.DESTROY(true);
  }
}
...
export class Background extends Entity {
  constructor(start_x: number, start_y: number, spr: string, dep: number) {
    super(start_x, start_y);
    this.sprite = spr;
    this.depth = dep;
  }
