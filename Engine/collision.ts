			...
			switch (this.collider_type) {
				case COLLIDERTYPE_POINT:
					...
					switch (other_collider.collider_type) {
						case COLLIDERTYPE_POINT:
							if (Math.round(x) == Math.round(otherx) && Math.round(y) == Math.round(othery)) return_data.push(other_data);
							break;

						case COLLIDERTYPE_RECTANGLE:
							if (PointInsideRectangle(x, y, otherx, othery, otherx + other_collider.width, othery + other_collider.height)) return_data.push(other_data);
							break;

						case COLLIDERTYPE_CIRCLE:
							if (PointInsideCircle(x, y, otherx, othery, other_collider.width)) return_data.push(other_data);
							break;
				...
				case COLLIDERTYPE_RECTANGLE:
					...
					switch (other_collider.collider_type) {
						case COLLIDERTYPE_POINT:
							if (PointInsideRectangle(otherx, othery, x, y, x + this.width, y + this.height)) return_data.push(other_data);

						case COLLIDERTYPE_RECTANGLE:
							if (rectangle_inside_rectangle(x, y, x + this.width, y + this.height, otherx, othery, otherx + other_collider.width, othery + other_collider.height)) return_data.push(other_data);

						case COLLIDERTYPE_CIRCLE:
							if (rectangle_inside_circle(x, y, x + this.width, y + this.height, otherx, othery, other_collider.width)) return_data.push(other_data);
				...
				case COLLIDERTYPE_CIRCLE:
					...
					switch (other_collider.collider_type) {
						case COLLIDERTYPE_POINT:
							if (PointInsideCircle(otherx, othery, x, y, this.width)) return_data.push(other_data);

						case COLLIDERTYPE_RECTANGLE:
							if (rectangle_inside_circle(otherx, othery, otherx + other_collider.width, othery + other_collider.height, x, y, this.width)) return_data.push(other_data);

						case COLLIDERTYPE_CIRCLE:
							if (circle_inside_circle(otherx, othery, other_collider.width, x, y, this.width)) return_data.push(other_data);
					...
					if (this.height > 0) {
						...
						do {
							...
							switch (other_collider.collider_type) {
								...
								case COLLIDERTYPE_RECTANGLE:
									if (PointInsideRectangle(check.x, check.y, otherx, othery, otherx + other_collider.width, othery + other_collider.height)) {
										other_data.hit.x = check.x;
										other_data.hit.y = check.y;
										return_data.push(other_data);
										continue;
								...
								case COLLIDERTYPE_CIRCLE:
									if (PointInsideCircle(check.x, check.y, otherx, othery, other_collider.width)) {
										other_data.hit.x = check.x;
										other_data.hit.y = check.y;
										return_data.push(other_data);
										continue;
...
export class ColliderRayCast extends ColliderPoint {
	...
	constructor(id: string, xoff: number, yoff: number, angle: number, dist: number) {
...
export function GetStaticCollision(x: number, y: number): StaticCollisionData {
	...
	if (!Game.active_scene || Game.active_scene.static_collision_map.length <= 0) return new StaticCollisionData(0, x, y, x, y, x, y);
	if (Math.floor(y / Game.active_scene.static_col_resolution) >= Game.active_scene.static_collision_map.length) return new StaticCollisionData(0, x, y, x, y, x, y);
	let submap = Game.active_scene.static_collision_map[Math.floor(y / Game.active_scene.static_col_resolution)];
	if (!submap || submap.length <= 0) return new StaticCollisionData(0, x, y, x, y, x, y);
	if (Math.floor(x / Game.active_scene.static_col_resolution) >= submap.length) return new StaticCollisionData(0, x, y, x, y, x, y);
